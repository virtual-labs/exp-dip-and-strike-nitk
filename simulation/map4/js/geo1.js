...
var questions = {
  ...
  setAns: function (ans) {
    if (simsubscreennum == 8) {
      if (soilType == "Fine grained soil") questions.ans1 = 3;
      else if (soilType == "Sandy soil") questions.ans1 = 2;
    } else questions.ans1 = ans;
  },
  frameQuestions: function (qun) {
    ...
    else document.getElementById("divq").innerHTML = qun;
...
$(function () {
  ...
  $("#solidtable td").on("click", function () {
    ...
    if (
      ...
      generateQuestion(
        q1,
        "________dotted lines and _______solid lines are identified in this map",
        "",
        "11 & 10",
        "9 & 12",
        "11 & 9",
...
function gameLoop() {
  ...
  if ((grid_size * ball.y) / 100 >= -75 && (grid_size * ball.y) / 100 <= -67) {
    ...
    ball.x += xunits * 1;
    ball.y += yunits * 1;
  } else if ((grid_size * ball.y) / 100 < -stopPoint) drawScreen();
}
...
function drawTunnel110() {
  ...
  if (tun2x > 1510) {
    ...
    generateQuestion(
      q2,
      "A 20m depth horizontal tunnel is proposed at an altitude of _______m from x axis parallel to it, between bounds.:",
      "",
      "120",
      "1500",
      "1200",
